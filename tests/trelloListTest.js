...
Scenario('test lists - it should return all attributes', function () {
...
Scenario('test lists - it should return the correct type', function () {
...
Scenario('test lists - it should return all lists attributes', function () {
...
Scenario('test lists - it should return the correct list attribute types', function () {
    getListsResponse.data.lists.forEach(function (currentList) {
...
Scenario('test lists - lists should have the same board id as the board', function () {
    getListsResponse.data.lists.forEach(function (currentList) {
        expect(currentList.idBoard).to.equal(getListsResponse.data.id);
    });
}).tag('@lists');

Scenario('test lists - lists should have the correct names', function (trelloData) {
    getListsResponse.data.lists.forEach(function (currentList) {
        expect(trelloData.LIST.LIST_NAMES).to.include(currentList.name);
    });
}).tag('@lists');

Scenario('test lists - lists should not be closed', function () {
    getListsResponse.data.lists.forEach(function (currentList) {
        expect(currentList.closed).to.equal(false);
    });
}).tag('@lists');

Scenario('test lists - softLimits should not be set', function () {
    getListsResponse.data.lists.forEach(function (currentList) {
        expect(currentList.softLimit).to.be.null;
    });
}).tag('@lists');
